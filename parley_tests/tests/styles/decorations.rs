...
fn style_overline() {
    ...
    let mut builder = env.ranged_builder(text);
    builder.push_default(StyleProperty::Overline(true));
    let mut layout = builder.build(text);
    layout.break_all_lines(None);
    layout.align(None, Alignment::Start, AlignmentOptions::default());
...
#[test]
fn style_overline_offset_values() {
    let mut env = TestEnv::new(test_name!(), None);
    let text = samples::LATIN;
    ...
    for offset in [-2.0, 0.0, 2.0, 4.0] {
        let mut builder = env.ranged_builder(text);
        builder.push_default(StyleProperty::Overline(true));
        builder.push_default(StyleProperty::OverlineOffset(Some(offset)));
        let mut layout = builder.build(text);
        layout.break_all_lines(None);
        layout.align(None, Alignment::Start, AlignmentOptions::default());
...
fn style_overline_size_values() {
    ...
    for size in [0.5, 1.0, 2.0, 4.0] {
        let mut builder = env.ranged_builder(text);
        builder.push_default(StyleProperty::Overline(true));
        builder.push_default(StyleProperty::OverlineSize(Some(size)));
        let mut layout = builder.build(text);
        layout.break_all_lines(None);
        layout.align(None, Alignment::Start, AlignmentOptions::default());
...
#[test]
fn style_overline_across_line_break() {
    let mut env = TestEnv::new(test_name!(), None);
    let text = samples::LATIN_MULTILINE;
...
#[test]
fn style_overline_partial_text() {
    let mut env = TestEnv::new(test_name!(), None);
    let text = "Hello World Test";

    let mut builder = env.ranged_builder(text);
    // Overline only "World"
    builder.push(StyleProperty::Overline(true), 6..11);
    let mut layout = builder.build(text);
    layout.break_all_lines(None);
    layout.align(None, Alignment::Start, AlignmentOptions::default());
